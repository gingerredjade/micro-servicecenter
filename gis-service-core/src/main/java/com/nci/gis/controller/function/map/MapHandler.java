 ...
 */
public class MapHandler implements DataProcFunction<Object> {

	private static final Logger _logger = LoggerFactory.getLogger(MapHandler.class);
	...
	public Object processData(HttpServletRequest request, AppParams __appParams, AuxParams __auxParams) throws Exception {
		...
		String URLStr = buildRequestURL(
			...
			request);
		_logger.info("【地图显示服务】Current Request URL = [{}].", URLStr);
		...
		try {
			...
			_logger.error("【地图显示服务】URL Request failed,url=[{}].error info=[{}]", URLStr, e.getMessage());
			throw new Exception(e.getMessage());
		}

		_logger.info("【地图显示服务】URL Request success,[{}].", URLStr);
		return resultData;
	}


	/**
	 * 构建地图显示服务后端实际服务的请求URL
	 *
	...
	private static String buildRequestURL(String gisServerUrl, String svcMapping, HttpServletRequest request) {
		...
		String alias = paramsMap.get("alias")[0];
		String version = paramsMap.get("version")[0];
