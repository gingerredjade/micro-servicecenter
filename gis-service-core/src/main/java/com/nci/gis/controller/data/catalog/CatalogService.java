package com.nci.gis.controller.data.catalog;
...
public class CatalogService {
	...
	@Autowired
	public CatalogService(GisAppServiceConfig gisAppServiceConfig) {
		this.gisAppServiceConfig = gisAppServiceConfig;
	}

	public CatalogOuputData execCatalog(
		String org,
		String format, String type,
		String alias, String version,
		String number, String auxParams,
		HttpServletRequest request) {

		// 0-- 遍历配置获取服务供应商对应的GIS服务器信息
		List<Map<String, String>> catalogConfigArr = gisAppServiceConfig.getGscatalog();
		String[] confArr = getServerConf(catalogConfigArr, org);
		...
		if (SERVER_PREFIX.isEmpty()) {
			...
			return buildCatalogOutputData(StateCodes.STATE_CODE_FAILED, errStr);
		} else {
		...
		appParams.put(SERVICE_MAPPING_IDENTITY.value, CATALOG_SERVICE.value);
		...
		try {
			...
			_logger.error("【网络分析服务】URL Request failed,org=[{}],format=[{}],type=[{}],alias=[{}],version=[{}],number=[{}],auxParams=[{}]."
				+ e.getMessage(), org, format, type, alias, version, number, auxParams);
	 ...
	 * @return		返回值
	 */
	private static CatalogOuputData buildCatalogOutputData(int code, String msg) {
		CatalogOuputData catalogOuputData = new CatalogOuputData();
		catalogOuputData.setCode(code);
		catalogOuputData.setMsg(msg);
		return catalogOuputData;
	}
	 ...
	 * @return		返回值
	 */
	private static CatalogOuputData buildCatalogOutputData(int code, String msg, String data) {
		CatalogOuputData catalogOuputData = new CatalogOuputData();
		catalogOuputData.setCode(code);
		catalogOuputData.setMsg(msg);
		catalogOuputData.setResultBody(data);
		return catalogOuputData;
	}
