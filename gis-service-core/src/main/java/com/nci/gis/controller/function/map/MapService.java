...
import com.nci.constants.ImageType.IMAGE_TYPE_BMP;
import com.nci.constants.ImageType.IMAGE_TYPE_JPG;
import com.nci.constants.ImageType.IMAGE_TYPE_PNG;
import com.nci.constants.ImageType.IMAGE_TYPE_UNKNOWN;
...
public class MapService {
	...
	private static final Logger _logger = LoggerFactory.getLogger(MapService.class);
	...
	@Autowired
	public MapService(GisAppServiceConfig gisAppServiceConfig) {
		this.gisAppServiceConfig = gisAppServiceConfig;
	}

	public String execMap(String org, String format, String alias, String version,
						  String bound, String layers, String width, String height,
						  String imageType, String transparent, String bgcolor, String auxParams,
		...
		try {
			...
			_logger.info("Params=[org={},format={},alias={},version={},bound={},layers={},width={},height={}," +
					"imageType={},transparent={},bgcolor={},auxParams={}]",
				org, format, alias, version, bound, layers, width, height, imageType, transparent, bgcolor, auxParams);
			_logger.error(e.getMessage());
			return buildMapOutputData(StateCodes.STATE_CODE_FAILED, e.getMessage()).toString();
		}*/
		...
		List<Map<String, String>> mapConfigArr = gisAppServiceConfig.getGsmap();
		String[] confArr = getServerConf(mapConfigArr, org);
		...
		if (SERVER_PREFIX.isEmpty()) {
			...
			return buildMapOutputData(StateCodes.STATE_CODE_FAILED, errStr).toString();
		} else {
		...
		getMap(request, response, SERVER_PREFIX, SERVER_HANDLER_CLASS);
		return null;
	}
	...
	private static void getMap(
		...
		switch (imageType) {
			case "0":
				type = IMAGE_TYPE_UNKNOWN.str;
				break;

			case "1":
				type = IMAGE_TYPE_PNG.str;
				break;

			case "2":
				type = IMAGE_TYPE_JPG.str;
				break;

			case "4":
				type = IMAGE_TYPE_BMP.str;
				break;
		...
		appParams.put(SERVICE_MAPPING_IDENTITY.value, SERVICE_MAPPING_MAP);
		...
		try {
			...
			IOException e) {
			String errStr = "【地图显示服务-Map资源】URL Request failed." + e.getMessage();
			_logger.error(errStr);
		}
	...
	/**
	 * 根据状态码、消息内容构建地图显示服务输出数据结果
	 *
	 * @param code	状态码
	 * @param msg	消息内容
	 * @return		返回值
	 */
	private static MapOutputData buildMapOutputData(int code, String msg) {
		MapOutputData mapOutputData = new MapOutputData();
		mapOutputData.setCode(code);
		mapOutputData.setMsg(msg);
		return mapOutputData;
	}


	/**
	 * 根据状态码、消息内容、数据构建地图显示服务输出数据结果
	 *
	 * @param code	状态码
	 * @param msg	消息内容
	 * @param data 	数据内容
	 * @return		返回值
	 */
	private static MapOutputData buildMapOutputData(int code, String msg, String data) {
		MapOutputData mapOutputData = new MapOutputData();
		mapOutputData.setCode(code);
		mapOutputData.setMsg(msg);
		mapOutputData.setResultBody(data);
		return mapOutputData;
	}
