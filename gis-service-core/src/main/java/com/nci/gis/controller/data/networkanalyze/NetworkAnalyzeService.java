 ...
 */
@Service
public class NetworkAnalyzeService {

	private static final Logger _logger = LoggerFactory.getLogger(NetworkAnalyzeService.class);
	...
	@Autowired
	public NetworkAnalyzeService(GisAppServiceConfig gisAppServiceConfig) {
		this.gisAppServiceConfig = gisAppServiceConfig;
	}

	public NetworkAnalyzeOutputData execNetwork(
		String org,
		String format, String alias, String version,
		String points, String srs, String avoidPoints,
		String passPoints, String naviMode, String auxParams,
		...
		try {
			...
			return buildNetworkAnalyzeOutputData(StateCodes.STATE_CODE_FAILED, e.getMessage());
		}*/

		// 0--3 遍历配置获取服务供应商对应的GIS服务器信息
		List<Map<String, String>> networkanaConfigArr = gisAppServiceConfig.getGsnetworkanalyze();
		String[] confArr = getServerConf(networkanaConfigArr, org);
		...
		if (SERVER_PREFIX.isEmpty()) {
			...
			return buildNetworkAnalyzeOutputData(StateCodes.STATE_CODE_FAILED, errStr);
		} else {
		...
		appParams.put(SERVICE_MAPPING_IDENTITY.value, NETWORKANALYSE_SERVICE.value);
		...
		try {
			...
			InvocationTargetException e) {
			_logger.error("【网络分析服务】URL Request failed,org=[{}],format=[{}],alias=[{}],version=[{}],points=[{}],srs=[{}]," +
				"avoidPoints=[{}],passPoints=[{}],naviMode=[{}],auxParams=[{}]."
				+ e.getMessage(), org, format, alias, version, points, srs, avoidPoints, passPoints, naviMode, auxParams);
			String errStr = "【网络分析服务】URL Request failed." + e.getMessage();
			return buildNetworkAnalyzeOutputData(StateCodes.STATE_CODE_FAILED, errStr);
		}
		 ...
		 * 3-- 封装应答
		 */
		return buildNetworkAnalyzeOutputData(
	 ...
	 * @return		返回值
	 */
	private static NetworkAnalyzeOutputData buildNetworkAnalyzeOutputData(int code, String msg) {
		NetworkAnalyzeOutputData networkAnalyzeOutputData = new NetworkAnalyzeOutputData();
		networkAnalyzeOutputData.setCode(code);
		networkAnalyzeOutputData.setMsg(msg);
		return networkAnalyzeOutputData;
	}


	/**
	 * 根据状态码、消息内容、数据构建网络分析服务输出数据结果
	 *
	 * @param code	状态码
	 * @param msg	消息内容
	 * @param data	数据内容
	 * @return		返回值
	 */
	private static NetworkAnalyzeOutputData buildNetworkAnalyzeOutputData(int code, String msg, String data) {
		NetworkAnalyzeOutputData networkAnalyzeOutputData = new NetworkAnalyzeOutputData();
		networkAnalyzeOutputData.setCode(code);
		networkAnalyzeOutputData.setMsg(msg);
		networkAnalyzeOutputData.setResultBody(data);
		return networkAnalyzeOutputData;
	}
