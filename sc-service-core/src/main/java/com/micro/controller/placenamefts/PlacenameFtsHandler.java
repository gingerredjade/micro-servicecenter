...
public class PlacenameFtsHandler implements DataProcFunction<String> {
	...
	public String processData(HttpServletRequest request, AppParams __appParams, AuxParams __auxParams) throws Exception {
		...
		String URLStr = buildRequestURL(
			...
			request);
		_logger.info("【地名检索服务】Current Request URL = [{}].", URLStr);
		...
		try {
			...
			_logger.error("【地名检索服务】URL Request failed,url=[{}].error info=[{}]", URLStr, e.getMessage());
			throw new Exception(e.getMessage());
		}

		_logger.info("【地名检索服务】URL Request success,[{}].", URLStr);
		return resultData;
	}


	/**
	 * 构建地名检索服务后端实际服务的请求URL
	 *
