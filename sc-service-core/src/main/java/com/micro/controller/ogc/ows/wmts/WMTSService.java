...
import com.micro.common.ClassUtils;
import com.micro.common.dynamic.classloader.SpringContextUtil;
import com.micro.conf.GisAppServiceConfig;
import com.micro.constants.*;
...
public class WMTSService {
	...
	public String execWmts(String org, String service, String request, String version, String format,
		...
		if (SERVER_PREFIX.isEmpty()) {
			String errStr = "Web服务器服务请求串前缀为空，请检查server-prefix配置";
	...
	private static void getWmtsCapabilities(
		...
		try {
			if (springMode) {
				...
				// 2--3 获取对象的方法
				Class<?>[] paramTypes = ClassUtils.getParameterClass(beanObj, PROCESS_METHOD);
				...
				// 2--3 获取对象的方法
				Class<?>[] parameters = ClassUtils.getParameterClass(impl, PROCESS_METHOD);
